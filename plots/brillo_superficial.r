...
HH<-read.table("tab_gal_gru.dat"); VG<-read.table("compact_in_voids_m3_full")
...
mu_no<-S$sigv;  R_no<-S$rabs1 ; N_no=length(R_no)

mu_gg<-SS$sigv; R_gg<-SS$rabs1 ; N_gg=length(R_gg);

mu_fi<-FF$sigv; R_fi<-FF$rabs1 ; N_fi=length(R_fi)

mu_cp<-Gf$sigv; R_cp<-Gf$rabs1 ; N_cp=length(R_cp)
...
plot(colmed_f,FL_f,type='l',ylim=c(0,630),xlim=c(-20,-23.5),xlab=labx,ylab=laby,
...
polygon(c(colmed_f,rev(colmed_f)),c(FL_f+error_f,rev(FL_f-error_f)),col=rgb(1, 101/255, 0,0.5),border=NA)
...
polygon(c(colmed_no,rev(colmed_no)),c(FL_no+error_no,rev(FL_no-error_no)),col=rgb(1,0,0,0.5),border=NA)
...
polygon(c(colmed_cp,rev(colmed_cp)),c(FL_cp+error_cp,rev(FL_cp-error_cp)),col=rgb(0,0,1,0.5),border=NA)
...
polygon(c(colmed_vv,rev(colmed_vv)),c(FL_vv+error_vv,rev(FL_vv-error_vv)),col=rgb(192/255,192/255,192/255,0.7),border=NA)


legend('topleft',c("Node", "Filaments",'Groups','Field','Voids'),bty="n",lty=c(1,1,1,1,1), col=c('red',naranja,"magenta",'darkblue','black'),horiz=FALSE,inset=0,cex=1.2,pch=c(16,18,17,20,21))
